...
output_path = "/Users/annie/emap/20180201/"

# read in dataframe with all data
all = readRDS(all_rds)
...
clusters_df <- clusters_df[which((clusters_df$mutant %in% strong_muts)),]

# additional filter to only include genes for which we have data
genes_with_data <- c("RNA1", "SRM1", "MOG1", "YRB1", "PSE1")
clusters_df <- clusters_df[which((clusters_df$protein %in% genes_with_data)),]

# will use statistical test to determine which edges to include; for testing abs val corr of corr > 0.3
clusters_df <- clusters_df[which(abs(clusters_df$corr.value) > 0.3),]

# exclude merged clusters
#clusters_df <- filter(clusters_df, grepl("_GO_", cluster))
...
quantile(as.numeric(as.character(deg_btw_all_df$degree)), na.rm=TRUE)
...
deg_btw_all_df$degree <- as.numeric(as.character(deg_btw_all_df$degree))
deg_btw_all_df$betweenness <- as.numeric(as.character(deg_btw_all_df$betweenness))
deg_btw_all_df_by_cluster <- group_by(deg_btw_all_df, cluster)
summary_deg_btw_all_df_by_cluster <- dplyr::summarize(deg_btw_all_df_by_cluster, 
                                    mean.degree = mean(degree),
                                    median.degree = median(degree),
                                    max.degree = max(degree),
                                    mean.betweennes = mean(betweenness),
                                    median.betweennes = median(betweenness) )
