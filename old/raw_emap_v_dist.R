...
interface$residue_protein <- paste(interface$index, "-", interface$protein)
all$residue <- parse_number(all$mutant.y)
all$residue_protein <- paste(all$residue, "-", all$protein)
all <- merge(all, interface, by="residue_protein", all=TRUE)
...
names(all)[names(all) == "protein.y"] <- "protein.interface"
